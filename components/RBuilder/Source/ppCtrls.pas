...
{******************************************************************************}
  ...
  {@TppDBText.SuppressRepeatedValues
    ...
    containing addresses.)  If the detail band overflows onto a new page, you
    can force the suppressed value to print again by using the
...
uses
  ppEngine, ppBands, ppRegion, ppPrintr, ppPlainText, ppRotatedText;
...
begin
  ...
  if Text = '' then
    lClientRect := Rect(mmLeft, mmTop, 5000, 5000)
  else
    lClientRect := TppRotatedText.CalcRotatedTextBounds(lCanvas, Text, Font, mmLeft, mmTop, mmWidth, mmHeight, Angle, AutoSize, Alignment, Border);
...
begin
  ...
  if WordWrap then
    begin
      ...
      TppPlainText.WordWrap(lDrawText.Text, PrintPosRect, lDrawText.TextAlignment, lDrawText.AutoSize, Font, FCharWrap, liLeading, lTabStops, lPrinter, liCharPos, lDrawText.WrappedText);
    end;
...
begin
  ...
  if not(lbSuppressWhenNotReprintOnSubsequent and (FPriorValue = cIgnore)) and (ResetOnCompute or (FPriorValue <> FFieldValue)) then
    begin
...
begin
  ...
  if (FSuppressRepeatedValues and not(FReprintOnSubsequent) and (ResetGroup <> nil)) then
    FPriorValue := cIgnore;
...
{$IFDEF Delphi6}
      if (FBlankWhenZero and (lValue <> Null)) and (VarType(lValue) in [varSmallInt..varCurrency, varShortInt..varInt64, varDate]) and (lValue = 0) then
{$ELSE}
      if (FBlankWhenZero and (lValue <> Null)) and (VarType(lValue) in [varSmallInt..varCurrency, varDate]) and (lValue = 0) then
{$ENDIF}
        lsText := ''
      else if (VarType(lValue) = varString) and (lValue = '') then
        lsText := ''
      else if (lDataType in [dtDate, dtDateTime]) and (VarIsNull(lValue)) then
    ...
    begin
      ...
      if (FBlankWhenZero) and (ldValue = 0) then
        ...
        begin
          ...
          if (lDataType = dtDateTime) and (ldValue = 0) then
            lsText := ''
          else
...
begin
 ...
 if (GeneratingNewPage) or (Report.SecondPass) then
   Inc(FImageID);
...
begin
  ...
  if (aDrawCommand is TppDrawImage) and (DataPipeline <> nil) then
    begin
      lsImageName := Name + DataPipeline.Name + DataField + IntToStr(DataPipeline.RecordNo);

      TppDrawImage(aDrawCommand).ImageName := lsImageName;
    end;
...
begin
  ...
  if (CompareText(aPropName, 'Position') = 0) or (CompareText(aPropName, 'LinePosition') = 0) then
    Byte(aValue) := Ord(TppLine(aObject).Position)

  else if (CompareText(aPropName, 'Style') = 0) then
    Byte(aValue) := Ord(TppLine(aObject).Style)
...
begin
  ...
  if (CompareText(aPropName, 'Shape') = 0) then
    Byte(aValue) := Ord(TppShape(aObject).Shape)
...
begin
  ...
  if (CompareText(aPropName, 'Divisor') = 0) then
    ...
    Byte(aValue) := Ord(TppDBCalc(aObject).DBCalcType)
