...
begin
  ...
  FCanvas.Brush.Style := bsSolid;
  if (FViewer <> nil) then
    FCanvas.Brush.Color := FViewer.Color
  else
...
begin
  ...
  if (FViewer = nil) or (FViewer.ZoomSetting = zs100Percent) then
    ...
    begin
      ...
      FScaledPageHeight := FViewer.ScrollBox.Height - liDeadArea;
      FScaledPageWidth := Round(ldRatio * FScaledPageHeight);

      {reduce width & height until entire page fits in viewable area}
      if (FPageImage.Width > (liDeadArea*2)) and (FPageImage.Height > (liDeadArea*2)) then
        while (FScaledPageWidth >= FPageImage.Width) do
          begin
            FScaledPageHeight := FScaledPageHeight - liDeadArea;
            FScaledPageWidth := Round(ldRatio * FScaledPageHeight);
          end;

      Result := Round((FScaledPageHeight / lPageDef.spHeight) * 100);
    end
  else   {zsPageWidth}
    begin
      {compute page rectangle size}
...
{------------------------------------------------------------------------------}
{ TppScreenDevice.RenderRectangle }

procedure TppScreenDevice.RenderRectangle(aDrawShape: TppDrawShape);
var
  liSize: Integer;
  liPosition: Integer;
begin
...
{------------------------------------------------------------------------------}
{ TppScreenDevice.DrawBorder }

procedure TppScreenDevice.DrawBorder(aDrawCommand: TppDrawCommand; aDrawRect: TRect);
var
...
begin
  ...
  lPenWidth := Round((aDrawCommand.Border.Weight * Screen.PixelsPerInch / 72) * FScaleX);
  ...
  for liOffset := 0 to lPenWidth do
    begin
...
{------------------------------------------------------------------------------}
{ TppScreenDevice.DrawRotatedText }

procedure TppScreenDevice.DrawRotatedText(aDrawText: TppDrawText);
var
...
begin
  ...
  FCanvas.Font.Height := Round(aDrawText.Font.Height * ScaleY);
  ...
  lCanvasTextOrigin.X := aDrawText.DrawLeft + Round(liTextBoxLeft * ScaleX);
  lCanvasTextOrigin.Y := aDrawText.DrawTop + Round(liTextBoxTop * ScaleY);
  ...
  if aDrawText.AutoSize then
    begin
      ...
      liWidth := Round((Trunc(ppFromMMThousandths(lDrawMMRect.Right, utScreenPixels, pprtHorizontal, nil))) * ScaleY);
      liHeight := Round((Trunc(ppFromMMThousandths(lDrawMMRect.Bottom, utScreenPixels, pprtHorizontal, nil))) * ScaleX);
...
{------------------------------------------------------------------------------}
{ TppScreenDevice.DrawStandardText }
...
begin
  ...
  liBorderOffset := Round(aDrawText.Border.SPWeight * FScaleX);
  liBorderBuffer := 0;
  ...
  for liLine := 0 to liLines do
    begin
      ...
      if aDrawText.Border.Visible then
        begin
          if aDrawText.IsMemo then
            liBorderBuffer := Round(ppToScreenPixels(0.5, utMillimeters, pprtHorizontal, nil) * FScaleX);
