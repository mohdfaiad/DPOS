...
begin
  ...
  if (Sender = FNew) then
    ...
    Screen.ActiveForm.Perform(WM_CLOSE, 0, 0);
...
begin
  ...
  if (Sender = FPortrait) and not(FPortrait.Checked) then
    DialogController.Report.PrinterSetup.Orientation := poPortrait

  else if (Sender = FLandscape) and not(FLandscape.Checked) then
...
begin
  ...
  if (Sender = FInches) then
    DialogController.MainReport.Units := utInches
  else if (Sender = FMillimeters) then
    DialogController.MainReport.Units := utMillimeters
  else if (Sender = FPrinterPixels) then
    DialogController.MainReport.Units := utPrinterPixels
  else if (Sender = FScreenPixels) then
    DialogController.MainReport.Units := utScreenPixels
  else if (Sender = FMMThousandths) then
    DialogController.MainReport.Units := utMMThousandths;
