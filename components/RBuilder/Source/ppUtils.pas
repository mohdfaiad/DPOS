...
uses
  ...
  SysUtils, TypInfo, Dialogs, Forms, ppTypes, ppDisplayFormat;
...
{$ENDIF}
  ...
  TppBorder = class(TPersistent)
    private
      ...
      procedure BeforePropertyChange(aPropertyName: String);
...
{$ENDIF}
  ...
  function  ppGetEnumStringAsByte(aEnumType: PTypeInfo; const aEnum: String): Byte;
...
var                 
  ...
  uDebugLog: TppDebugLog = nil;
  uResourceManager: TppResourceManager = nil;
  uFinalized: Boolean;
  uIsWinXP: Boolean;
...
begin
  ...
  lFontInfo.FontName := StrPas(LogFontEx.elfLogFont.lfFaceName);
  ...
  {build list of unique font names}
  if lStrings.Count = 0 then
    lbAddFont := True
  else
    lbAddFont := AnsiCompareText(lStrings[lStrings.Count-1], lFontInfo.FontName) <> 0;
  ...
  if lbAddFont then
    ...
    begin
      ...
      TppFontInfo(lStrings.Objects[lStrings.Count-1]).CharSet := DEFAULT_CHARSET;
    end;
...
begin
  ...
  if (FLanguage <> lSaveLanguage) then
    FResourceFilesLoaded := False;
...
begin
  ...
  liPenWidth := Round(aBorder.Weight * Screen.PixelsPerInch / 72);
  aCanvas.Pen.Width := liPenWidth;
  ...
  if bpTop in aBorder.BorderPositions then
    begin
      aCanvas.MoveTo(aRect.Left, aRect.Top);
      aCanvas.LineTo(aRect.Right + liPenWidth, aRect.Top);
    end;

  if bpRight in aBorder.BorderPositions then
    begin
      aCanvas.MoveTo(aRect.Right + liPenWidth, aRect.Top);
      aCanvas.LineTo(aRect.Right + liPenWidth, aRect.Bottom);
    end;
...
procedure TppBorder.DoOnChange;
begin
...
begin
  Result := Round(FWeight * Screen.PixelsPerInch / 72);
...
procedure TppBorder.BeforePropertyChange(aPropertyName: String);
var
...
begin
  ...
  if ((VarType(aVar1) = varString) and (VarType(aVar2) = varString)) or
     ((VarType(aVar1) = varOleStr) and (VarType(aVar2) = varOleStr)) then
     Result := AnsiCompareText(aVar1, aVar2)
...
begin
  ...
  liIndex := lValues.IndexOf(aEnum);

  if liIndex = -1 then
    Result := 255  {Byte range is 0..255}
  else
    Result := liIndex;

  lValues.Free;

end; {ppGetEnumStringAsByte}
...
begin
  ...
  liLength := GetSystemDirectory(PChar(Result), MAX_PATH);
  SetLength(Result, liLength);

end; {function, ppGetSystemDirectory}
...
begin
  ...
  if  ((uResourceManager = nil) and not(uFinalized)) then
    uResourceManager := TppResourceManager.Create;
