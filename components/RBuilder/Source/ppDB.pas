...
type
  ...
  {@TppField
    ...
    Field components are currently used by all CustomDataPipeline descendants
    (TppJITPipeline and TppTextPipeline.)  The TppBDEPipeline does not
    (and cannot) use TppField components because it is not a CustomDataPipeline
    descendant.  The TppBDEPipeline does not need TppField components because it
    retrieves Fields from standard Delphi datasets.}
...
begin
  ...
  while (Result = nil) and (liIndex < FieldCount) do
    ...
    if ppEqual(Fields[liIndex].FieldName, aFieldName) then
...
begin
  ...
  while (Result = nil) and (liIndex < FieldCount) do
    ...
    if ppEqual(Fields[liIndex].FieldAlias, aFieldAlias) then
      Result := Fields[liIndex]
    else
      Inc(liIndex);
