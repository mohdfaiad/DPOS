...
type
  ...
  {@TppDesigner.EnableHelp
    ...
    This property controls whether or not the Report Designer will make calls to
    WinHelp when the user clicks F1.}
  ...
  {@TppDesigner.HelpFile
    ...
    end-user reporting solution. When the user clicks F1 in the Report Designer,
    this property determines the help file which will be opened by WinHelp.
    HelpFile can contain either a file name or a full path and file name.}
  ...
  {@TppDesigner.IniStorageName
    ...
    ($WINSYS)\\RBuilder.ini. The ($WINSYS) is used to specify the Windows
    system directory.}
  ...
  {@TppDesigner.IniStorageType
    ...
    IniFile	        Stores user preferences to the .ini file specified by
                    ...
                    the .ini file will be located in the Windows system
                    directory.
...
OnCreateComponent event.}
  ...
  {@TppDesigner.ShowComponents
    ...
    *Not available in Delphi 1, ** Available in Delphi 1 and 2 only if TeeChart
    is installed. By default all values are contained in the set.

    Note: This property can only be set at run-time in Delphi 1.0}
...
begin
  if Assigned(FOnGetDisplayFormats) then FOnGetDisplayFormats(Sender, TStrings(aParameters));
end;
...
begin
  ...
  lbValid := True;
  if Assigned(FOnValidateComponent) then FOnValidateComponent(Sender, lEventParams.Component, lbValid);
