...
uses
  ...
  ppClass, ppComm, ppTypes, ppTreeVw, ppToolWn, ppUtils,
  daDataView, daForms,
...
begin
  ...
  if (Parent is TScrollbox) then
    begin
      ...
      if (lScrollBox.HorzScrollBar.Range < FDataView.Left) then
        lScrollBox.HorzScrollBar.Range :=  FDataView.Left + FDataView.Width + 10;

      if (lScrollBox.VertScrollBar.Range < FDataView.Top) then
    ...
    begin
      ...
      if (FLinkItem <> nil) then
        begin
          ...
          if ((lField.Linkable) and not(lField.IsDetail)) then
            DoOnSelectField
          else
            FLinkItem := nil;
        end;
