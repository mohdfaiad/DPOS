...
type
  ...
  protected
    ...
    procedure ehLayoutManager_LoadEnd(Sender, aParameters: TObject);
...
begin
  ...
  FDesignTime := (aOwner <> nil) and (aOwner.ClassName = 'TppDesignWindow');

  ppUtils.gbDesignTime := FDesignTime;
...
begin
  ...
  FEventHub.ReportEvents.mcLoadEnd.AddNotify(ehLayoutManager_LoadEnd);
  ...
  if (FNoteBook.PageCount >= 2) then
    begin
      FNotebook.ActivePageIndex := FNoteBook.PageCount-1;
      FNotebook.ActivePageIndex := FNoteBook.PageCount-2;  // make sure that the page will be visible.
...
procedure TppDesignerWindow.ehLayoutManager_LoadEnd(Sender, aParameters: TObject);
begin
  ...
  if (aParameters = Report) and (FNoteBook.PageCount > 1) and (FNoteBook.ActivePageIndex <> FNotebook.PageCount-2) then
    FNotebook.ActivePageIndex := FNoteBook.PageCount-2;
...
begin
  ...
  if (FNotebook.ActivePageIndex <> FNotebook.PageCount-2) then
    FNotebook.ActivePageIndex := FNotebook.PageCount-2;
...
begin
  ...
  else if (FNotebook.ActivePageIndex = FNotebook.PageCount-1) then   //  Preview
     FNotebook.ActivePageIndex := FNotebook.PageCount-2;   // select Design page
...
begin
  ...
  liWindowState := lIniStorage.ReadInteger('Designer Position', 'WindowState', Ord(wsNormal));
...
begin
  ...
  if lDesignModule.Active = aActive then Exit;
...
{------------------------------------------------------------------------------}
{ TppDesignerWindow.WMClose}

procedure TppDesignerWindow.WMClose(var Message: TWMClose);
begin


  if InternalCloseQuery then
    inherited;
