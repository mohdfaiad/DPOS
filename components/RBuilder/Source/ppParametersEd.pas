...
uses
  ...
  StdCtrls, Buttons, Grids, ExtCtrls, Typinfo,
  ppDsIntf, ppComm, ppTypes, ppRTTI,
  ppParameter;
...
{------------------------------------------------------------------------------}
{ TppParameterEditor.ParameterToControls }

procedure TppParameterEditor.ParameterToControls;
var
  lParameter: TppParameter;
begin

  if (FParameterList <> nil) and not(csDesigning in FParameterList.ComponentState) then
    begin

      if (cbxDataType.Items.Count = 0) then
        TppParameterDataTypeInfo.GetEnumNames(cbxDataType.Items);

      if (lbxParameter.ItemIndex >= 0) and (lbxParameter.ItemIndex < lbxParameter.Items.Count) then
        begin
          lParameter := TppParameter(lbxParameter.Items.Objects[lbxParameter.ItemIndex]);
          edtName.Text := lParameter.Name;
          cbxDataType.Text := GetEnumName(TypeInfo(TppDataType), Ord(lParameter.DataType));
          edtValue.Text := lParameter.AsString;
        end
      else
        begin
          edtName.Text := '';
          cbxDataType.Text := 'dtString';
          edtValue.Text := '';
        end;
...
begin
  ...
  if (lbxParameter.SelCount > 0) and (lbxParameter.Items.Count > 0) then
    SetObjectInspectorToSelection(True)
  else
