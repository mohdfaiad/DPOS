...
begin
  ...
  if (Source is TdaLinkBroker) then
    begin
      for liIndex := 0 to TdaLinkBroker(Source).LinkCount - 1 do
        begin
          lLink := TdaSQLLink(TdaLinkBroker(Source).Links[liIndex]);
...
procedure TdaLinkBroker.CreateLink(aMasterField, aDetailField: TdaField);
var
  lSQL: TdaSQL;
  lMasterSQL: TdaSQL;
  liIndex: Integer;
...
begin
  ...
  lDetailField := lSQL.GetFieldForAlias(aSQLLink.DetailField.Alias);
  ...
  if (lMasterSQL <> nil) then
    begin
      lMasterField := lMasterSQL.GetFieldForAlias(aSQLLink.MasterField.Alias);
...
begin
  ...
  if (lMasterSQL <> nil) and (lMasterSQL.IsOrderedByCalcField) then
    begin
      Result := False;
