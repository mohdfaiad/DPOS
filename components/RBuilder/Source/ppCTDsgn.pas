...
begin
  ...
  FSortAsc.Tag := 0;
  FSortAsc.OnClick := ehItem_Click;

  FSortDesc := AddButton();
  FSortDesc.ImageIndex := ToolImageList.AddIcon('PPSORTDESC');
  FsortDesc.Tag := 1;
  FSortDesc.OnClick := ehItem_Click;
  ...
  FLeftJustify.OnClick := ehItem_Click;
  ...
  FRightJustify.OnClick := ehItem_Click;
...
begin
  ...
  else if (Sender = FSortAsc) then
    SortButtonClickEvent(Sender)

  else if (Sender = FSortDesc) then
    SortButtonClickEvent(Sender)

  else if (Sender = FBold) then
    FontChangeEvent(Sender)
  ...
  else if (Sender = FLeftJustify) then
    AlignmentButtonClickEvent(Sender)

  else if (Sender = FCenter) then
    AlignmentButtonClickEvent(Sender)

  else if (Sender = FRightJustify) then
    AlignmentButtonClickEvent(Sender);
...
{------------------------------------------------------------------------------}
{ TppCTFormatToolbar.FontChangeEvent }

procedure TppCTFormatToolbar.FontChangeEvent(Sender: TObject);
var
  lFont: TFont;
  lbChecked: Boolean;
begin
