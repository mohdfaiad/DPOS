...
type
  ...
  {@TppDBArchiveReader
    ...
    opened, previewed and printed using this component. Use the DataPipeline
    property to indicate where the archives are stored. Use the NameField to
    specify the database field where the name of each archive is stored. Use
    the BLOBField to specify the database field where actual archive itself
    (i.e. the binary data) is stored. Once these properties have been set, use
    the Name property to specify the archive that should be loaded when Print is
    called.}
