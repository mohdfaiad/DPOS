...
begin
  ...
  lDrawCommand.Clickable := Assigned(FOnDrawCommandClick) or IsActiveNotify(ciPrintableDrawCommandClick);
...
begin
  ...
  if (CompareText(aPropName, 'Alignment') = 0) then
    Byte(aValue) := Ord(TppPrintable(aObject).Alignment)
    ...
    Byte(aValue) := Ord(TppPrintable(aObject).DefaultPropEditType)
    ...
    Byte(aValue) := Ord(TppPrintable(aObject).Units)
