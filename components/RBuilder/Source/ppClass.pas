...
type
  ...
  public
    ...
    procedure ResolvePipelineReferences;
  ...
  {@TppComponent.Anchors
    ...
    It is possible to anchor a component to more than one side of the parent
    component as long as the anchors are not opposites.  For example, by default
    each new report component is anchored to the top and left of its parent
    component.  Anchoring a component to the Top and Bottom simultaneously is
    not valid. }
...
begin
  ...
  try
    ...
    for liIndex := 0 to lPipelineList.Count-1 do
      lPipelineList.GetPipeline(liIndex).ResolvePipelineReferences(lPipelineList);
  
    {resolve report datapipeline reference}
    if (FDataPipeline = nil) then
      begin
        lDataPipeline := lPipelineList.GetPipelineForComponentName(FDataPipelineName);
        SetDataPipeline(lDataPipeline);
      end;
  
    {resolve band pipeline references}
    for liIndex := 0 to BandCount - 1 do
      Bands[liIndex].ResolvePipelineReferences(lPipelineList);
  
    {resolve group pipeline references}
    for liIndex := 0 to GroupCount - 1 do
      Groups[liIndex].ResolvePipelineReferences(lPipelineList);
    ...
    for liIndex := 0 to FModules.Count-1 do
      begin
        ...
        if (csDesigning in ComponentState) or not(Self is TppReport) or
           ((Self is TppReport) and not(pppcEmpty in lModule.DesignState)) then
          Proc(lModule);
      end;
...
begin
  ...
  if (FReport <> nil) then
    begin
//      SendEventNotify(Self, ciReportBandRemoved, nil);  // free design control
  ...
  if FReport <> nil then
    begin
//      SendEventNotify(Self, ciReportBandAdded, nil);  // create design control
    ...
    begin
      ...
      for liObject := liStart to FObjects.Count-1 do
        begin
          ...
          // send notification to the design control
          lComponent.SendEventNotify(lComponent, ciComponentBringToFront, nil);

        end;

       SendDesignMessage(RM_COMPONENTUPDATED, ppWCustomMsg,  FObjects[aNewIndex]);
...
begin
  ...
  SendDesignMessage(RM_COMPONENTUPDATED, ppWCustomMsg, aObject);
...
begin
  ...
  FObjects.Insert(0, aObject);
   ...
   // send notification to the design control
  lComponent := TppComponent(aObject);
  lComponent.SendEventNotify(lComponent, ciComponentSendToBack, nil);
...
begin
  ...
  {notify report designer }
  SendDesignMessage(RM_COMPONENTCHANGE, ppWCustomMsg, Self);
...
begin
  ...
  if (atLeft in FAnchors) then
    liNewBounds.Left := mmLeft + liMoveLeft;

  if (atTop in FAnchors) then
    liNewBounds.Top := mmTop + liMoveTop;

  if (atRight in FAnchors) then
    liNewBounds.Left := mmLeft + liMoveRight;

  if (atBottom in FAnchors) then
...
begin
  ...
  if ppEqual(aPropName, 'AbsolutePage')  then
    ...
    Byte(aValue) := Ord(TppCustomReport(aObject).PassSetting)
    ...
    Byte(aValue) := Ord(TppCustomReport(aObject).Units)
...
begin
  ...
  if ppEqual(aPropName, 'Caption') then
    ...
    Byte(aValue) := Ord(TppBand(aObject).Units)
...
begin
  ...
  if ppEqual(aPropName, 'BreakComponent') then
    ...
    Byte(aValue) := Ord(TppGroup(aObject).BreakType)
    ...
    Byte(aValue) := Ord(TppGroup(aObject).Units)
...
begin
  ...
  if ppEqual(aPropName, 'Band')  then
    ...
    Byte(aValue) := Ord(TppComponent(aObject).BandType)
