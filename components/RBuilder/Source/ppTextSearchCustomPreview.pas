        ...
        begin
          ...
          if (aEventID = ciDevicePageReceive) then
            begin
              ...
              FStatusBar.Panels[0].Text := lsMessage;
            end
...
begin
  ...
  {the status bar in the designer needs to be restored since there are 5 panels
  in the status bar of the designer as opposed to two panels in the standard
  print preview form.}
  if FViewer.DesignViewer then
    begin

      if (FStatusBar.Panels.Count > 2) then
        begin
          FStatusBar.Panels[1].Width := 90;
          FStatusBar.Panels[0].Width := FStatusBar.Width -
                                       (FStatusBar.Panels[1].Width + FStatusBar.Panels[2].Width +
                                        FStatusBar.Panels[3].Width + FStatusBar.Panels[4].Width) - 20;
        end
      else
        begin
          FStatusBar.Panels[1].Width := 90;
...
begin
  ...
  if (FStatusBar = nil) then
    raise ESearchError.Create('TppCustomSearchPreview.BeforePreview: Must assign status bar first.');
  ...
  if (FStatusBar <> nil) then
    begin
      if FStatusBar.SimplePanel then
        raise ESearchError.Create('TppCustomSearchPreview.StatusChange: Must have a non SimplePanel status bar to support text search.');
...
{@TppCustomSearchPreview.SetStatusBar }

procedure TppCustomSearchPreview.SetStatusBar(aStatusBar: TStatusBar);
begin
...
begin
  if (FViewer.ScreenDevice.Publisher <> nil) and FViewer.ScreenDevice.Publisher.ReportCompleted then
    StatusBar.Panels[0].Text := Viewer.CurrentPage.PageSetDescription
  else
    StatusBar.Panels[0].Text := Viewer.CurrentPage.PageNoDescription;
end;
