...
begin
  ...
  liBuffer := Round(Printer.PixelsPerInch.X * (0.01));
  ...
  FTextWidthPoints := TppPDFUtils.PixelsToPoints(liTextWidthPixels, Printer.PixelsPerInch.X);
  FTextHeightPoints := TppPDFUtils.PixelsToPoints(liTextHeightPixels, Printer.PixelsPerInch.Y);
  ...
  case lAlignment of
    ...
    taRightJustify: FLeftOffset := FLeftOffset + ldOffset;
  end;
...
begin
  ...
  lsText := TppPDFUtils.ProcessText(DrawTextCommand.Text);
