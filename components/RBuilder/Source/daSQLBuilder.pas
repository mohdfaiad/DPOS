...
type
  ...
  protected
    function AddTableJoin(const aNameOrAlias, aJoinToNameOrAlias: String; aJoinType: TdaJoinType; const aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean; virtual;
  ...
  public
    ...
    function AddJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean; overload; virtual;
    function AddFullOuterJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean; virtual;
    function AddLeftOuterJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean; virtual;
    function AddRightOuterJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean; virtual;
  ...
  public
    ...
    function AddExpression(const aExpression: String): TdaCalculation; virtual;
    function Remove(const aTableNameOrAlias: String; const aFieldNameOrAlias: String): TdaField; overload; virtual;
...
{@TdaSQLSelectTables.AddJoin
  ...
  example:
    ...
    <Table>
     ...
     aJoinToNameOrAlias          existing table to be joined to
...
function TdaSQLSelectTables.AddJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean;
begin
  Result := AddTableJoin(aNameOrAlias, aJoinToNameOrAlias, dajtInner, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases);
...
function TdaSQLSelectTables.AddFullOuterJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean;
begin
  Result := AddTableJoin(aNameOrAlias, aJoinToNameOrAlias, dajtOuter, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases);
...
function TdaSQLSelectTables.AddLeftOuterJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean;
begin
  Result := AddTableJoin(aNameOrAlias, aJoinToNameOrAlias, dajtLeftOuter, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases);
...
function TdaSQLSelectTables.AddRightOuterJoin(const aNameOrAlias, aJoinToNameOrAlias, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean;
begin
  Result := AddTableJoin(aNameOrAlias, aJoinToNameOrAlias, dajtRightOuter, aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases);
...
function TdaSQLSelectTables.AddTableJoin(const aNameOrAlias, aJoinToNameOrAlias: String; aJoinType: TdaJoinType; const aFieldNamesOrAliases, aOperators, aJoinToFieldNamesOrAliases: String): Boolean;
var
  liIndex: Integer;
  liField1: Integer;
  liField2: Integer;
...
begin
  ...
  {make sure that both tables exists}
  liTable1 := IndexOf(aNameOrAlias);
  liTable2 := IndexOf(aJoinToNameOrAlias);

  if (liTable1 < 0) and (IndexOfAvailable(aNameOrAlias) < 0) then Exit;

  if (liTable2 < 0) and (IndexOfAvailable(aJoinToNameOrAlias) < 0) then Exit;

  {add tables, if needed}
  if liTable1 >= 0 then
    lTable1 := Items[liTable1]
  else
    lTable1 := Add(aNameOrAlias);

  if liTable2 >= 0 then
    lTable2 := Items[liTable2]
  else
    lTable2 := Add(aJoinToNameOrAlias);
  ...
  for liIndex := 0 to lFieldNames1.Count - 1 do
    begin
      ...
      liField1 := lSelectFields.IndexOf(lTable1.TableName, lsFieldName1);

      if (liField1 >= 0) then
        lField1 := lSelectFields[liField1]
      else
        begin
          liField1 := lSelectFields.IndexOfAvailable(lTable1.TableName, lsFieldName1);
          if (liField1 >= 0) then
            lField1 := lSelectFields.Available[liField1];
        end;

      liField2 := lSelectFields.IndexOf(lTable2.TableName, lsFieldName2);

      if (liField2 >= 0) then
        lField2 := lSelectFields[liField2]
      else
        begin
          liField2 := lSelectFields.IndexOfAvailable(lTable2.TableName, lsFieldName2);
          if (liField2 >= 0) then
            lField2 := lSelectFields.Available[liField2];
        end;

      if (lField1 <> nil) and (lField2 <> nil) then
        begin
          lJoinOperator := daStringToJoinOperator(lOperators[liIndex]);
          lTable1.AddTableJoin(lTable2, lField2.FieldName, lField1.FieldName,  lJoinOperator);
...
{@TdaSQLCalcFields.AddExpression
  Adds a expression calc field to the Items[] array. }

function TdaSQLCalcFields.AddExpression(const aExpression: String): TdaCalculation;
begin
...
begin
  ...
  if (CompareText(aMethodName, 'Clear') = 0) then
    ...
    begin
      ...
      aParams.SetParamValue(1, Integer(lsExpression));
    end
  else if (CompareText(aMethodName, 'Remove') = 0) then
    begin
      aParams.GetParamValue(0, lsTableNameOrAlias);
...
initialization
  RegisterClasses([TdaSQLBuilder, TdaSQLTableList, TdaSQLSelectTables,
                   TdaSQLFieldList, TdaSQLSelectFields, TdaSQLCalcFields, TdaSQLGroupByFields,
                   TdaSQLOrderByFields, TdaSQLCriteriaList]);
...
finalization
  ...
  UnRegisterClasses([TdaSQLBuilder, TdaSQLTableList, TdaSQLSelectTables,
                   TdaSQLFieldList, TdaSQLSelectFields, TdaSQLCalcFields, TdaSQLGroupByFields,
                   TdaSQLOrderByFields, TdaSQLCriteriaList]);
