...
type
  ...
  {@TppSystemVariable.VarType
    ...
    *Note: Most reports which display page counts should have the
    Report.PassSetting property set to psTwoPass.  This guarantees an accurate
    page count when the report is being previewed.}
...
begin
  ...
  else if ValidNotify(aCommunicator, FCalcComponent, aEventID, FCalcEventType) then
    Compute

  else if (aCommunicator = Band.Report.Engine) and (aEventID = ciDetailOutOfSpace) and
  ...
  if (aCommunicator = Band.Report.Engine) and (aEventID = ciPageBackward) then
    FReverseNav := True;
...
begin
  if (DataType = dtString) and (FReverseNav) then
    begin
      Compute;
      FReverseNav := False;
    end;
...
begin
  ...
  case FVarType of
    ...
    vtPageSet, vtPageSetDesc, vtPageNoDesc, vtDocumentName:
      begin
        lValue := GetStandardValue;
        lsText := lValue;
      end;
...
begin
  ...
  if (CompareText(aPropName, 'AsBoolean') = 0) then
    ...
    Byte(aValue) := Ord(TppVariable(aObject).CalcType)

  else if (CompareText(aPropName, 'DataType') = 0) then
    Byte(aValue) := Ord(TppVariable(aObject).DataType)

  else if (CompareText(aPropName, 'ResetComponent') = 0) then
    Integer(aValue) := Integer(TppVariable(aObject).ResetComponent)

  else if (CompareText(aPropName, 'ResetType') = 0) then
    Byte(aValue) := Ord(TppVariable(aObject).ResetType)
...
begin
  ...
  if (CompareText(aPropName, 'VarType') = 0) then
    ...
    Byte(aValue) := Ord(TppSystemVariable(aObject).DataType)
