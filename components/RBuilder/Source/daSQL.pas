...
type
  ...
  TdaSQL = class (TppRelative)
    ...
    public
      ...
      function IsOrderedByCalcField: Boolean;
  ...
  {@TdaField.Alias
    Defaults to the value of the FieldAlias. However, when the same field is a
    dded to the SQL multiple times a suffix is appended to differentiate the
    field.
  ...
  {@TdaField.FieldAlias
   ...
   the Alias is derived from the FeildName.}
  ...
  TdaCalculation = class (TdaField)
    ...
    public
      ...
      function SelectSQLString: String; override;
      function SelectSQLStringWithoutAlias: String;
  ...
  TdaSQLLink = class (TdaChild)
    private
      ...
      FDetailSQLName: String;
      FMasterSQLName: String;
      function GetDescription: String;

    protected
      procedure SaveComponents(Proc: TGetChildProc); override;
      procedure AssignSQLNames(const aDetailSQLName, aMasterSQLName: String);
    ...
    public
      ...
      property DetailSQLName: String read FDetailSQLName;
      property LinkColor: TColor read FLinkColor write FLinkColor;
      property MasterField: TdaField read FMasterField;
      property MasterSQLName: String read FMasterSQLName;
...
uses
  ...
  daJoinTableDlg, daLinkBroker, daMagicSQL, daMetaDataManager;
...
begin
  ...
  if (Source is TdaSQLLink) then
    begin
      ...
      FDetailSQLName := TdaSQLLink(Source).DetailSQLName;
      FMasterSQLName := TdaSQLLink(Source).MasterSQLName;

      if ((FDetailSQLName = '') or (FMasterSQLName = '')) and (Parent <> nil) then
        begin
          lSQLObject := GetSQL;

          if (lSQLObject.MasterSQL <> nil) then
            lsSQLName := lSQLObject.MasterSQL.Name

          else
            lsSQLName := lSQLObject.Name;

          AssignSQLNames(lSQLObject.Name, lsSQLName);
        end;
    end;
...
{------------------------------------------------------------------------------}
{ TdaSQLLink.AssignSQLNames }

procedure TdaSQLLink.AssignSQLNames(const aDetailSQLName, aMasterSQLName: String);
begin

  FDetailSQLName := aDetailSQLName;
  FMasterSQLName := aMasterSQLName;

end;  {procedure, AssignSQLNames}
    ...
    begin
      ...
      for liIndex2 := 0 to lTable.TableJoinCount - 1 do
        begin
          ...
          if (lTableJoin.LocalField = nil) or (lTableJoin.ForeignField = nil) then
...
begin
  ...
  {We alias all Interbase tables because of IB quirk when joining table multiple times}
  if (IsInterbase) then
    Result := Result + '_1';
...
begin
  ...
  if (MasterSQL <> nil) then
    lsSQLName := MasterSQL.Name

  else
    lsSQLName := Name;

  Result.AssignSQLNames(Name, lsSQLName);
...
begin
      ...
      if (aField is TdaCalculation) then
        begin
          ...
          if (TdaCalculation(aField)).MagicallyTweaked then
            ...
            lsAlias := daRemoveInvalidAliasing(TdaCalculation(aField).SelectSQLStringWithoutAlias, lbInvalid);
        end
  ...
  if (IsAdvantage and (lsAlias <> '') and (lsAlias[1] in cNumerics)) then
    lsAlias := 'cw' + lsAlias;
  ...
  while not(Result) and (liIndex < CalcFieldCount) do
    begin
      ...
      if CalcFields[liIndex].CalcType in [dacaCount, dacaAvg, dacaMax, dacaMin, dacaSum] then
        Result := True
      else if (CalcFields[liIndex].CalcType = dacaExpression) then
        begin
          lsExpression := UpperCase(CalcFields[liIndex].Expression);

          if (Pos('SUM(', lsExpression) > 0) or
             (Pos('AVG(', lsExpression) > 0) or
             (Pos('MIN(', lsExpression) > 0) or
             (Pos('MAX(', lsExpression) > 0) or
             (Pos('COUNT(', lsExpression) > 0) then
             Result := True;
        end;
...
begin
  ...
  if FDataDictionary.GetJoinData(aTable1.RawTableName, aTable2.RawTableName, lsJoinType, lFieldNames1, lFieldNames2, lOperators) then
    begin
      aTable2.JoinType := TdaJoinType(ppGetEnumStringAsByte(TypeInfo(TdaJoinType), lsJoinType));
  ...
  case aCriteria.CriteriaType of
    ...
    dacrField:
      begin
        ...
        else if (aCriteria.Field is TdaCalculation)  then
          lsField := TdaCalculation(aCriteria.Field).SelectSQLStringWithoutAlias
            ...
            begin
              ...
              while (liIndex2 < CalcFieldCount) and not(lbFound) do
                begin
                  ...
                  if (CompareText(lCalculation.Expression, lCriteria.Field.SelectSQLString) = 0) then
                    lbFound := True
                  else
                    Inc(liIndex2);
...
{------------------------------------------------------------------------------}
{ TdaSQL.IsOrderedByCalcField }

function TdaSQL.IsOrderedByCalcField: Boolean;
var
  liIndex: Integer;
begin
...
end; {function, IsOrderedByCalcField}
...
begin
  ...
  if (FMasterSQL <> nil) and (FMasterSQL.IsOrderedByCalcField) then
    begin
...
{------------------------------------------------------------------------------}
{ TdaCalculation.SelectSQLString }

function TdaCalculation.SelectSQLString: String;
var
  lSQL: TdaSQL;
  lsAliasOperator: String;
begin
  ...
  {if the SQLField name is not the same as the FieldName, this field needs to be aliased}
  if (lSQL.IsMSAccess) or (lSQL.IsAdvantage) then
    lsAliasOperator := ' AS '
  else
    lsAliasOperator := ' ';
...
{------------------------------------------------------------------------------}
{ TdaCalculation.SelectSQLStringWithoutAlias}

function TdaCalculation.SelectSQLStringWithoutAlias: String;
begin
...
end; {function, SelectSQLStringWithoutAlias}
...
begin
  ...
  if (aValue <> FExpression) then
    if (ExpressionIsFieldName(aValue)) then
      begin
        if (UpperCase(aValue) <> UpperCase(FField.SQLString)) then
...
begin
  ...
  if (FField <> nil) then
    begin
      ...
      lOldField.Name := lOldField.GetValidName(lOldField);
  ...
  lField2.Name := lField2.GetValidName(lField2);
  lField2.Assign(lField);
...
{------------------------------------------------------------------------------}
{ TdaCriteria.ExpressionIsFieldName }
...
begin
  ...
  while not(Result) and (liIndex < lSQL.SelectFieldCount) do
    if (UpperCase(aValue) = UpperCase(lSQL.SelectFields[liIndex].SQLString)) then
      Result := True
    else
      Inc(liIndex);

end; {procedure, ExpressionIsFieldName}
...
begin
  ...
  while not(lbFound) and (liIndex < lSQL.SelectFieldCount) do
    begin
      ...
      if (lField.FieldName = FField.FieldName) and (lField.TableAlias = FField.TableAlias) then
        lbFound := True
      else
        Inc(liIndex);
...
begin
  ...
  if (Length(aName) > 0) and (aName[1] in cNumerics) then
    Result := 1
    ...
    while (Result = 0) and (liIndex <= Length(aName)) do
      begin
        ...
        if (aName[liIndex] in cInvalidChar) and
           { Interbase field names can contain '$' }
           not((aDatabaseType = dtInterbase) and (aName[liIndex] = '$')) then
          Result := liIndex
        else
          Inc(liIndex);
...
function daStringToJoinType(const aJoinType: String): TdaJoinType;
begin
  Result := TdaJoinType(ppGetEnumStringAsByte(TypeInfo(TdaJoinType), aJoinType));
...
begin
  ...
  liASPosition := Pos(' AS ', lsUpperCase);
  liCASTPosition := Pos('CAST', lsUpperCase);

  {we don't want to mistake "Castaway AS MyAlias" for a vaild string}
  lbValidCAST := ((Pos('CAST(', lsUpperCase) > 0) or (Pos('CAST ', lsUpperCase) > 0));

  if (liASPosition > 0) then
    {if 'CAST' is present and it appears before 'AS' we'll assume this is a CAST statement}
    if (liCASTPosition > 0) then
      aIsInvalid := (not((lbValidCAST) and (liCASTPosition < liASPosition)))
...
end; {function, daRemoveInvalidAliasing}
