...
begin
  ...
  FBevelCreate.SetBounds(0,5,167,116);
...
begin
  ...
  lSplitter.Parent  := FClientPanel;
  lSplitter.Width   := 2;
  lSplitter.Cursor  := crVSplit;
  lSplitter.Beveled := False;
  lSplitter.MinSize := 40;
  lSplitter.Align   := alBottom;
  lSplitter.Top := FInspectorPanel.Top - 1;
  lSplitter.AutoSnap := True;
  lSplitter.ResizeStyle := rsUpdate;
  lSplitter.ParentColor := True;
  ...
  {create horizontal splitter}
  lSplitter := TSplitter.Create(Self);
  lSplitter.Parent  := FReportPanel;
  lSplitter.Width   := 2;
  lSplitter.Cursor  := crVSplit;
  lSplitter.Beveled := False;
  lSplitter.Top := FReportTree.Top + FReportTree.Height+1;
...
begin
  ...
  if (FPopupMenu.Items[itReportTree] = Sender) then
    begin
      FReportTree.Visible := not(FReportTree.Visible);
    end
...
//    FInspector.SelectObject(FObjectTree.SelectedComponent);
    ...
    if (FObjectTree.SelectedComponent is TppBand) then
...
begin
  ...
  FReportTree.Visible := aIniStorage.ReadBool('ReportOutline', 'ReportOutlineVisible', FReportTree.Visible);
