...
{////////COMPONENT POPUP MENUS/////////}
  ...
  TppRichTextPopupMenu = class(TppCustomRichTextPopupMenu)
    private
      ...
      procedure LoadFromRTFStream(aStream: TMemoryStream);
      procedure SaveToRTFStream(aStream: TMemoryStream);
...
begin
  ...
  Result.LanguageIndex := uLanguageIndex;
...
begin
  ...
  if (uLanguageIndex <> aLanguageIndex) then
    begin
      uLanguageIndex := aLanguageIndex;
...
begin
  ...
  if (TraRTTI.GetPropRec(GetRefComponent.ClassType, aMenuItem.Name, lPropRec)) then
...
begin
  ...
  if (FComponent <> nil) and (FComponent is TppPrintable) then
    TppPrintable(FComponent).SendDesignMessage(aMsg, aWParam, aLParam);
...
begin
  ...
  ItemByName('MailMerge').Checked := lbMailMerge;
...
//  lRTFEditor.PopupParent := Screen.ActiveForm;
  ...
  SaveToRTFStream(lRichTextStream);
  ...
  if (lRTFEditor.ShowModal = mrOK) then
    begin
      ...
      LoadFromRTFStream(lRichTextStream);
...
procedure TppRichTextPopupMenu.SaveToRTFStream(aStream: TMemoryStream);
var
  lParams: TraParamList;
begin

  lParams := GetRTTIClass.GetParams('SaveToRTFStream');

  try
    lParams.SetValuePointer(0, @aStream);
    GetRTTIClass.CallMethod(Component, 'SaveToRTFStream', lParams, True);
  finally
    lParams.Free;
  end;
...
procedure TppRichTextPopupMenu.LoadFromRTFStream(aStream: TMemoryStream);
var
  lParams: TraParamList;
begin
  ...
  lParams := GetRTTIClass.GetParams('LoadFromRTFStream');

  try
    lParams.SetValuePointer(0, @aStream);
    GetRTTIClass.CallMethod(Component, 'LoadFromRTFStream', lParams, True);
  finally
    lParams.Free;
  end;
