...
type
  ...
  TppDBPipeline = class(TppCustomDataPipeline)
    ...
    {end of optional section}
      ...
      property MasterDataSource: TDataSource read GetMasterDataSource;
...
begin
  ...
  case GetFieldDataType(aFieldName) of
    ...
    dtLargeInt:
{$IFDEF Delphi6}
      Result := TLargeintField(FFieldDataLink.Field).AsLargeInt;
{$ELSE}
      {Delphi 5 variants do not support Int64}
      Result := TLargeintField(FFieldDataLink.Field).AsInteger;
{$ENDIF}
...
{$IFDEF Delphi10}
  if (FFieldDataLink.Field is TWideMemoField) then
    begin
      lsValue := TWideMemoField(FFieldDataLink.Field).Value;
      aStream.WriteBuffer(Pointer(lsValue)^, Length(lsValue));
    end
  else
{$ENDIF}
...
{$IFDEF Delphi10}
  if (FFieldDataLink.Field is TWideMemoField) then
    begin
      SetLength(lsValue, aStream.Size);
      aStream.ReadBuffer(Pointer(lsValue)^, aStream.Size);
      TWideMemoField(FFieldDataLink.Field).Value := lsValue;
    end
  else
