...
const
  ...
  cBandTypeClassMap: array[TppBandType] of TppBandClass =
                 (TppHeaderBand, TppTitleBand, TppColumnHeaderBand, TppGroupHeaderBand, TppDetailBand,
                  TppGroupFooterBand, TppColumnFooterBand, TppSummaryBand, TppFooterBand, TppPageStyle);
...
begin
  ...
  if (Engine <> nil) then
    begin
      ...
      EventNotifies := EventNotifies + [ciReportBeforeOpenDataPipelines, ciReportAfterOpenDataPipelines]
    end;
...
begin
  ...
  for liIndex := 0 to AutoSearchFieldCount - 1 do
    begin
      ...
      {ignore those fields where no criteria was entered by user in AS Dialog}
      if (AutoSearchFields[liIndex].SearchExpression <> '') then
