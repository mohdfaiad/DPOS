...
type
  ...
  {@TppProducer.Device
    ...
    This property discontinued as of version 4.0
    It is replaced by the DeviceType property. However, the full implementation
    of this property has been retained for backward compatiblity. If you have
    event handlers coded for a previous version of ReportBuilder which contain
    Device assignments, then these will continue to function normally. Also,
    report objects (saved in forms, files, or database fields) that have this
    property saved will update the DeviceType property accordingly when they
    loaded for the first time.

    The Device property determines where output goes when the Print method is
    called. These are the possible values:

    <Table>
    Value	        Meaning
    ----------    -----
    dvPrinter	    The Print Dialog is displayed and then output is directed to the
                  printer specified in the PrinterSetup property.
    dvScreen	    The Print Preview form is created and the first page of the
                  report is sent to the Viewer component of this form.
    dvFile	      The Print Dialog is displayed and then output is directed to the
                  file specified in the TextFileName property.
    dvArchive	    The Print Dialog is displayed and then output is directed to the
                  file specified in the ArchiveFileName property.
    </Table>

    Note: TppDeviceType is declared in the ppTypes unit.}

  {@TppProducer.DeviceType

    The DeviceType property determines where output goes when the Print method
    is called. These are the possible values:
    ...
    Note: DeviceType replaces the Device property.}
  ...
  {@TppProducer.Language
    ...
    translation. For more information see Deploying YourOwn Translation.}
...
begin
  ...
  if ((FPrintDialog <> nil) and (FPrintDialog.PrintToFile)) or
     ((FPrintDialog = nil) and (aDeviceClass.InheritsFrom(TppFileDevice) and
       not(DeviceClass.InheritsFrom(TppArchiveDevice)) )) then
    begin
      if (FPrintDialog <> nil) then
        aDeviceClass := ppDeviceClassForName(FPrintDialog.DeviceType);
      ...
      if (FPrintDialog <> nil) then
        ...
        Result.FileName := FTextFileName;
          ...
          ((FPrintDialog = nil) and (aDeviceClass.InheritsFrom(TppArchiveDevice))) then
    begin
      Result := TppArchiveDevice.Create(Self);
      Result.LanguageIndex := LanguageIndex;
...
begin
  ...
  FLanguageIndex := Ord(aLanguageType);
