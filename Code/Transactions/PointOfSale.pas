...
begin
  try
    ...
    Begin
          ...
          If  (qry_RelatedItemDetails.RecordCount >0)  Then  // «·«’‰«› «·„Ã„⁄…
          Begin
              SQLText := '';
...
begin
  ...
  If SDS_Header.State in [dsEdit,dsInsert] then Begin
      buttonSelected := MessageDlg('Â·  —Ìœ Õﬁ« Õ–› «·’‰› ø',mtInformation, mbOKCancel, 0);
        if buttonSelected = mrOK then
           SDS_Details.Delete;
   end;
end;
