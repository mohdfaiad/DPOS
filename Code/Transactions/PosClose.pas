...
begin
  try
    ...
    Begin
        ShowMessage('íÌÈ ÊÍÏíÏ äÞØÉ ÇáÈíÚ   ÞÈá ÇáÍÝÙ');
    ...
    Begin
        ShowMessage('íÑÌì ÊÍÏíÏ ÇáßÇÔíÑ ÞÈá ÇáÍÝÙ  ');
    ...
    if SDS_Header.ApplyUpdates(0) = 0 then Begin
     ...
     ShowMessage('ÍÏË ÎØÃ ÃËäÇÁ ÇáÍÝÙ') ;
    end;
  except
     ShowMessage('ÍÏË ÎØÃ ÃËäÇÁ ÇáÍÝÙ') ;
  end;
...
begin
  ...
  try
    // Show a confirmation dialog
    buttonSelected := MessageDlg('åá ÊÑíÏ ÍÞÇ ÍÐÝ ÇáÈíÇäÇÊ',mtError, mbOKCancel, 0);
    ...
    Begin
        SDS_Header.Delete;
        if SDS_Header.ApplyUpdates(0) = 0 then Begin
           ShowMessage('Êã ÇáÍÐÝ ÈäÌÇÍ');
           BtnOpenClick(Sender);
        end else Begin
         ShowMessage('ÍÏË ÎØÃ ÃËäÇÁ ÍÐÝ ÇáÈíÇäÇÊ') ;
         BtnOpenClick(Sender);
        end;
    end;
  except
      ShowMessage('ÍÏË ÎØÃ ÃËäÇÁ ãÓÍ ÇáÈíÇäÇÊ , áÇ íãßä ÍÐÝ ÈíÇäÇÊ ãÓÊÎÏãÉ');
      BtnOpenClick(Sender);
  end;
