...
type
  TfmWareHouse = class(TForm)
    ...
    SDS_HeaderWareHouseNameAr: TStringField;
    SDS_HeaderWareHouseNameEn: TStringField;
    GroupBox2: TGroupBox;
    BtnOpen: TButton;
    btnAdd: TButton;
    btnEdit: TButton;
...
procedure TfmWareHouse.BtnOpenClick(Sender: TObject);
begin
  SDS_Header.Close;
  SDS_Header.Open;
...
procedure TfmWareHouse.btnEditClick(Sender: TObject);
begin
  SDS_Header.Edit;
...
procedure TfmWareHouse.btnSaveClick(Sender: TObject);
...
begin
  ...
  If (SDS_HeaderWareHouseNameAr.AsString = '') AND (SDS_HeaderWareHouseNameEn.AsString = '')  Then
  Begin
     ShowMessage('ÌÃ»  ÕœÌœ «·«”„ ﬁ»· «·Õ›Ÿ');
     edtName.SetFocus;
     Exit;
  ...
  IsDuplicated := RepeatedKey('tbl_WareHouse', ' WareHouseCode = ''' + SDS_HeaderWareHouseCode.AsString + '''  ');
...
procedure TfmWareHouse.BtnCancelClick(Sender: TObject);
Var
...
procedure TfmWareHouse.btnDeleteClick(Sender: TObject);
Var
  buttonSelected : Integer;
begin
  try
...
procedure TfmWareHouse.FormCreate(Sender: TObject);
Begin
  {
...
procedure TfmWareHouse.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmWareHouse.btnAddClick(Sender: TObject);
begin
  SDS_Header.Append;
  btnEdit.Enabled := False;
  BtnOpen.Enabled := False;
...
procedure TfmWareHouse.SDS_HeaderBeforePost(DataSet: TDataSet);
begin
 SDS_HeaderCompanyCode.Value := DCompany;
end;

procedure TfmWareHouse.BtnShowClick(Sender: TObject);
var lkp : Tlkp;
begin
