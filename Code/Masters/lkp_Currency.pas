...
procedure TfmCurrency.BtnOpenClick(Sender: TObject);
begin
  SDS_Header.Close;
  SDS_Header.Open;
...
procedure TfmCurrency.btnEditClick(Sender: TObject);
begin
  SDS_Header.Edit;
...
procedure TfmCurrency.btnSaveClick(Sender: TObject);
Var IsDuplicated : Boolean;
begin
  If SDS_HeaderCurrencyCode.AsString = '' Then
  Begin
  ...
  If (SDS_HeaderCurrencyNameA.AsString = '') AND (SDS_HeaderCurrencyNameE.AsString = '')  Then
  Begin
     ShowMessage('ÌÃ»  ÕœÌœ «·«”„ ﬁ»· «·Õ›Ÿ');
     edtName.SetFocus;
     Exit;
  ...
  IsDuplicated := RepeatedKey('tbl_Currency', ' CurrencyCode = ''' + SDS_HeaderCurrencyCode.AsString + '''  ');
...
procedure TfmCurrency.BtnCancelClick(Sender: TObject);
Var
...
procedure TfmCurrency.btnDeleteClick(Sender: TObject);
Var
  buttonSelected : Integer;
begin
  try
...
procedure TfmCurrency.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmCurrency.btnAddClick(Sender: TObject);
begin
  SDS_Header.Append;
  btnEdit.Enabled := False;
  BtnOpen.Enabled := False;
...
procedure TfmCurrency.SDS_HeaderBeforePost(DataSet: TDataSet);
begin
 SDS_HeaderCompanyCode.Value := DCompany;
end;

procedure TfmCurrency.BtnShowClick(Sender: TObject);
var lkp : Tlkp;
begin
